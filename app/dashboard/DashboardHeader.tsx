...
export function DashboardHeader() {
	...
	const isHd = window.innerHeight <= 800;
	return (
					...
					isHd && (
						<nav className="flex items-center gap-6 text-sm mr-auto ml-4">
							<a href="#" className="relative pb-2 text-primary transition hover:text-helio-gold">
								Visão geral
								<span className="absolute inset-x-0 bottom-0 h-0.5 rounded-full bg-helio-gold" />
							</a>
							<a href="#" className="relative pb-2 text-secondary transition hover:text-helio-gold">
								Frota
								<span className="absolute inset-x-2 bottom-0 h-0.5 rounded-full bg-transparent transition group-hover:bg-helio-gold" />
							</a>
						</nav>
					)
				}
