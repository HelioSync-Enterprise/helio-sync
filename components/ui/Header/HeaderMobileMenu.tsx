...
export function HeaderMobileMenu({ isOpen, items, onClose }: HeaderMobileMenuProps) {
	return (
		<div
			className={`absolute top-[calc(100%+0.65rem)] left-1/2 -translate-x-1/2 w-[min(92vw,30rem)] 
				overflow-hidden rounded-3xl border border-white/10 bg-[linear-gradient(160deg,rgba(13,40,24,0.98),rgba(6,14,6,0.96))] 
				p-4 shadow-[0_20px_65px_-35px_rgba(0,0,0,0.95)] transition-all duration-350 
				lg:hidden ${isOpen? 'pointer-events-auto translate-y-0 opacity-100'	: 'pointer-events-none -translate-y-3 opacity-0'}`}>
			...
			<div className="relative z-10 flex flex-col gap-4">
				...
				<Link href="/login" className={`${buttonVariants({ variant: 'enter' })} w-full`} onClick={onClose}>
					Entrar
				</Link>
			</div>
		</div>
